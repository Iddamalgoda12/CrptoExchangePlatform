...
public:
	...
	OrderBook(std::string fileName);                      /**construct,reads csv file*/
	std::vector<std::string> getKnownProducts();                           /**return a vector of all known products eg)ETH/BTC , DOGE/BTC... in the data set*/
	std::vector<OrderBookEntry> getOrders(OrderBookType type,std::string product,std::string timestamp);     /**returns a vector of orders according to the sent filters*/
	std::string getEarliestTime();                                    /**returns the earliest time*/
